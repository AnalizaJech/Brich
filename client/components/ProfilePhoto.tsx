...
export default function ProfilePhoto({
  ...
  const generateAvatarUrl = (
    ...
    const nationalities = nationalityMap[
      personalityType as keyof typeof nationalityMap
    ] || ["us"];
  ...
  if (imageError) {
    return (
      <div
        className={`${sizeClasses[size]} ${className} rounded-full bg-gradient-to-br from-slate-400 to-slate-600 flex items-center justify-center`}
      >
  ...
  return (
    <div
      className={`${sizeClasses[size]} ${className} rounded-full overflow-hidden bg-white/10`}
    >
      <img
